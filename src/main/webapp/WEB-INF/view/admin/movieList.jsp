...
<body>
        ...
        <div id="page-wrapper">
            ...
            <div class="panel-heading"> 
                <h3 > 
                    网站配置&nbsp;&nbsp;
                    <small>电影列表</small> 
					<a class="btn btn-info pull-right" href="${pageContext.request.contextPath}/adminController/addMovie">添加电影</a>
                </h3> 
            </div>
            ...
            <div class="panel-body"> 
                ...
                <span class="col-md-6 col-md-push-6">
	                <form action="${pageContext.request.contextPath}/adminController/searchMovie" method="post">
				       <div class="input-group ">
				           	<input type="text" class="form-control" name="movieName" > 
				            <div class="input-group-btn">
				            <button class="btn btn-default ">搜索电影</button>
				            </div>
				       </div>
	                </form>
                </span>
            ...
            <table class="table table-striped table-bordered table-hover"> 
                    ...
                    <c:forEach var="movie" items="${movies}" varStatus="status"> 
                        <tr> 
                            <td>${movie.id}</td> 
                            <td><img src="${movie.poster}" width="100" referrerpolicy="never"></td> 
                            <td>${movie.name}</td>
                            <td>${movie.type}</td>
                            <td>${movie.published_year}</td>
                            <td>
	                            <a href="${pageContext.request.contextPath}/adminController/modifyMovie?id=${movie.id}&name=${movie.name}&poster=${movie.poster}&type=${movie.type}&published_year=${movie.published_year}" class="btn btn-info">修改</a>&nbsp;
	                            <a href="${pageContext.request.contextPath}/adminController/deleteMovie?id=${movie.id}&currentPage=${currentPage}" class="btn btn-info">删除</a>
                            </td>  
                        </tr> 
                     </c:forEach>
                     <tr id="message">
	...
	$(function(){
	    ...
	    var href = "/adminController/getMovies?currentPage=";
	    ...
	    if (pageCount <= 5 ) {//总页数小于五页，则加载所有页
	        ...
	        if (dqPage < 5) {//当前页小于5，加载1-5页
	            ...
	            if (dqPage <= pageCount-2) {//最后一页追加“...”代表省略的页
	            	item += "<li><a href='"+href+pageCount+"' >"+"..."+"</a></li>";
	            }
	            item+="<li><a href='"+href+(dqPage+1)+"' aria-label=\"Next\"><span aria-hidden=\"true\">&raquo;</span></a></li>";
	            $('#pageBtn').append(item);
	            return;
